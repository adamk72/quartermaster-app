...
- [ ] Container model overhaul: the current container concept may need to become a first-class object type to handle the full range of use cases. Containers include not just carried bags but also location-based caches and safe houses. Needs:
  - [ ] Optional location field (for caches, safe houses, etc.)
  - [ ] Ability to add/remove containers dynamically as the campaign progresses
  - [ ] Ownership: a container can be owned by a character, a pack animal, or no one (a stash at a location)
...
- [ ] Character enhancements:
  - [ ] Associate containers with characters: each character can own one or more containers (e.g. bags of holding). Owned containers should display with a character-scoped name in the inventory list (e.g. "Ayloc's Bag of Holding") so players know where items are stored.
  - [ ] Add max carrying weight to containers.
