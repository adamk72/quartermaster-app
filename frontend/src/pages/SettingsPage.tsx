  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <div className="w-48">
          <label className="block text-xs font-heading font-semibold text-parchment-dim mb-1">
            Name
          </label>
          <input
        ...
        <div className='flex items-center gap-3 mt-4.5'>
          <button
            type="submit"
            className="px-4 py-2 bg-gold text-base font-heading font-semibold rounded-lg hover:bg-gold-bright transition-colors text-sm shrink-0"
          >
            {label?.id ? "Update" : "Add"}
          </button>
          {label?.id && (
            <button
              type="button"
              onClick={onCancel}
              className="px-4 py-2 bg-surface text-parchment-dim border border-border rounded-lg hover:bg-card-hover transition-colors text-sm shrink-0"
            >
              Cancel
            </button>
          )}
          <span
            className="inline-block px-2.5 py-1 rounded text-xs font-medium shrink-0"
            style={{
              backgroundColor: hexWithAlpha(
                form.color ?? DEFAULT_LABEL_COLOR,
                "40",
              ),
        ...
        <div className="shrink-0">
          ...
          <div className="flex items-center gap-1">
            ...
            />
            <input
              className="input-themed !w-20 text-xs"
              value={form.color ?? ""}
              onChange={(e) => setForm({ ...form, color: e.target.value })}
              placeholder="#hex"
            />
...
export function SettingsPage() {
  ...
  const handleDelete = async (label: Label) => {
    if (
      !(await confirm(
        `Delete label "${label.name}"? It will be removed from all items.`,
      ))
    )
      return;
    try {
  ...
  const handleSaveInviteCode = async () => {
    ...
    try {
      ...
      toast.error(
        e instanceof Error ? e.message : "Failed to update invite code",
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div>
      <h2 className="font-heading text-3xl font-bold text-parchment mb-6">
        Settings
      </h2>

      <div className="bg-card border border-border rounded-xl p-6 space-y-4 max-w-xl mb-6">
        <div>
          <h3 className="font-heading text-xl font-semibold text-parchment mb-2">
            Invite Code
          </h3>
      ...
      <div className="bg-card border border-border rounded-xl p-6 space-y-6 w-3/4">
        <div>
          <h3 className="font-heading text-xl font-semibold text-parchment mb-4">
            Item Labels
          </h3>
          <p className="text-sm text-parchment-muted mb-4">
            Labels are color-coded tags that can be assigned to items. Each item
              ...
              {labels.map((label) => (
                ...
                >
                  ...
                  <span
                    ...
                    style={{
                      backgroundColor: hexWithAlpha(label.color, "40"),
